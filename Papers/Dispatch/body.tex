...
\EXP{(\mathrm{T}_{1}, \mathrm{T}_{2}, \ldots, \mathrm{T}_{m}) \unlhd (\mathrm{U}_{1}, \mathrm{U}_{2}, \mathinner{\ldotp\ldotp}, \mathrm{U}_{n})} if and only if
\EXP{m=n} and, for all \EXP{1\leq{}k\leq{}m}, \EXP{\mathrm{T}_{k} \unlhd \mathrm{U}_{k}}.  Objects, functions, and tuples
are all \emph{values}, and they all have type \TYP{Any}.
...
if \VAR{x} names a getter function.  Field declarations in objects,
...
\EXP{\VAR{equal}(p,\VAR{cp})} will simply ignore the color of \VAR{cp} when
...
to treat an ordinary point as if it its color were \TYP{Purple}.
This is easily done with Fortress functional methods,
and without having to modify the definition of \TYP{Point}:
\begin{codeexamplesize}
\begin{FortressCode}
\KWD{trait} \TYP{ColorPoint} \KWD{extends} \TYP{Point} \\
\2\+c\COLON \TYP{Color} \\
...
(and an argument of type \TYP{Point} in the first position).

It might be that other traits, completely unrelated to points
and colored points, also define functional methods
named \VAR{equal}, and it might also be that there are top-level
definitions of functions named \VAR{equal}.  All these are considered
...
\VAR{makePoint}(x\COLON \mathbb{R}, y\COLON \mathbb{R}) = \TYP{CartesianPoint}(x, y) \\
 \\
\KWD{object} \TYP{CartesianPoint}(x\COLON \mathbb{R}, y\COLON \mathbb{R}) \KWD{end} \\
 \\
\KWD{object} \TYP{PolarPoint}(\rho\COLON \mathbb{R}, \theta\COLON \mathbb{R}) \\
...
\KWD{end} \\
 \\
\KWD{object} \TYP{ColorPointObject}(x\COLON \mathbb{R}, y\COLON \mathbb{R}, c\COLON \TYP{Color}) \KWD{end} \\
 \\
...
But the API mentions only trait \EXP{\mathbb{Z}} and its one declaation of \VAR{asString}, so only
...
\begin{codeexamplesize}
\begin{FortressCode}
\OPR{API}\: \TYP{IntegerToStringFunction} \\
 \\
...
of type \EXP{\mathbb{Z}}, but it is clear that the functionality
...
then prepending a plus sign (the \KWD{asif} keyword indicates that
...
component \TYP{IntegerToStringFunction}, because that would completelt
...
\begin{codeexamplesize}
\begin{FortressCode}
\OPR{API}\: \TYP{IntegerToStringFunctionalMethod} \\
 \\
\KWD{trait} \mathbb{Z} \\
...
they all actually define the same name \VAR{m}.  Now suppose that m2, m3, and m4
...
The idea behind our implementation strategy is that if, for a given invocation of \VAR{m},
...
A type may be the name of a trait or object, a parenthesized type, a
...
in \secref{target}.  To summarize, the source language has
...
If an object declaration with name \EXP{\mathrm{U}} has a \NT{ParameterList}, say:
\begin{codeexamplesize}
\begin{tabbing}
...
If an object declaration with name \EXP{\mathrm{U}} has no \NT{ParameterList},
...
Note that the scope of a top-level function declaration or definition
