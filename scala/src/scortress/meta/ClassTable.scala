...
import scortress.definitions._
...
object ClassTable {
  ...
  private var table:Map[String, DfType] = new HashMap

  /**  */
  def initialize(prog:Program):Unit =
    table = new HashMap ++ prog.defns.filter(defn =>
        defn.isInstanceOf[DfType]).map(defn =>
          defn.asInstanceOf[DfType]).map(defn => (defn.name, defn))
