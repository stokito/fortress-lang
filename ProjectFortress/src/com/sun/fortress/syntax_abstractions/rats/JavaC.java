...
public class JavaC {
    ...
    public static int compile(String sourceDir, String destinationDir,
                              String filename) {
        Debug.debug(Debug.Type.SYNTAX, "compiling a temporary parser...");
        String classpath = sourceDir + PATHSEP
            + getFortressThirdPartyDependencyJars()
            + PATHSEP + getFortressBuildDir();
        String[] args = {"-nowarn", "-cp", classpath , "-d", destinationDir,
                         filename};
    ...
    private static String getFortressBuildDir() {
        return ProjectProperties.FORTRESS_AUTOHOME+File.separatorChar+
            "ProjectFortress"+File.separatorChar+"build"+File.separatorChar;
    }

    private static String getFortressThirdPartyDependencyJars() {
        String sepChar = ""+File.separatorChar;
        String thirdPartyDir = ProjectProperties.FORTRESS_AUTOHOME+
            File.separatorChar+"ProjectFortress"+File.separatorChar+
