...
public abstract class BaseTask extends FortressExecutable {
    ...
    // Tasks have a position in the current row of the pedigree tree,
    // a count that represents the id of it's next child, a depth (the
    // depth in the pedigree tree), an updated dot product for drandom
    // number generation, an internal counter storing how many drandom
    // numbers have been generated by this thread, and a name that is
    // the current pedigree.
    private int position;
    private int count; // This might have to be an AtomicInteger
    private int depth;
    private long dotprod; // Not the right type
    ...
    public BaseTask() {
        ...
        this.position = 1;
        this.count = 1;
        this.dotprod = MagicNumbers.a(this.depth) * this.count;
        this.randomCount = 0;
    ...
    public BaseTask(BaseTask parent) {
        ...
        this.count = parent.getAndIncrementCount();
        this.dotprod = 
            parent.getDotProduct() + 
            MagicNumbers.a(this.depth) * this.count; 
        this.randomCount = 0;
