...
public class TemplateGapNodeCreator extends CodeGenerator implements Runnable {
    ...
    public void generateAdditionalCode() {
      for (NodeClass nodeClass: ast.classes()) {
          generateTemplateGapNode(nodeClass);
      }      
    }

    private void generateTemplateGapNode(NodeClass node) {
        String className = "TemplateGap"+node.name();
        TabPrintWriter writer = options.createJavaSourceInOutDir(className);

        // Class header
        writer.startLine("/** A TemplateGap node " + node.name());
        writer.startLine(" **/");
    ...
    private void writeGetters(TabPrintWriter writer, String name) {
        writer.startLine("final public Id getId() { return _id; }");
