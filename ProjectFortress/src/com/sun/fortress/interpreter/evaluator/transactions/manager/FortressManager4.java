...
public class FortressManager4 extends BaseManager {
    ...
    private void sleepyTime() {
	    ...
	    long diff = stime - (long) sleepTime;
            FortressTaskRunner.debugPrintln("Sleeping done: time = expected = " + sleepTime + "ms real = " +  stime + "ms diff = " + diff);
        } else {
	    ...
	    long stime = System.currentTimeMillis() - current;
	    long diff = stime - (long) sleepTime;
            FortressTaskRunner.debugPrintln("Sleeping done expected = " + sleepTime + "ms real = " + stime + "ms diff = " + diff);
        }
    }
    ...
    public void pickOne(Transaction me, Transaction other) {
        ...
        if (winner == me) {
            if (me.isActive()) {
                if (!other.abort()) {
                    me.abort();
                    FortressTaskRunner.debugPrintln("Resolving conflict between me = " + me + " and other = " + other + " by special aborting me");
                    sleepyTime();
                } else {
                    FortressTaskRunner.debugPrintln("Resolving conflict between me = " + me + " and other = " + other + " by aborting other");
                }
            }
        } else {
            if (other.isActive()) {
                FortressTaskRunner.debugPrintln("Resolving conflict between me = " + me + " and other = " + other + " by aborting me");
                me.abort();
                sleepyTime();
            }
        }
    ...
    public void pickOne(Transaction me, Collection<Transaction> others) {
        ...
        if (winner == me) {
            ...
            boolean ilose = false;
            FortressTaskRunner.debugPrintln("Resolving conflict between me = " + me + " and others = " + others + " by aborting them");
            
            for (Transaction t : others) {
                if (t != me) {
                    t.abort();
                    if (!t.isAborted() || !t.isOrphaned())
                        ilose = true;
                }
            }
//          if (ilose) {
...
//          }
            ...
            me.abort();
            FortressTaskRunner.debugPrintln("Resolving conflict between me = " + me + " and other = " + others + " by aborting me");
            sleepyTime();
        }
    }
