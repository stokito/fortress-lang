...
public class IndexBuilder {
	...
	public ApiIndex buildApiIndex(Api ast, long modifiedDate) {
		...
		final Map<IdName, Grammar> grammars =
			new HashMap<IdName, Grammar>();
	 ...
	 */
	 ...
	 private void buildMethod(FnAbsDeclOrDecl ast,
		 ...
		 */
		 private void buildGrammar(GrammarDecl ast, Map<IdName, Grammar> grammars) {
			 final IdName name = ast.getName().getName();
			 //final Map<IdName, Production> productions = new HashMap<IdName, Production>();
			 NodeAbstractVisitor_void handleDecl = new NodeAbstractVisitor_void() {
				 @Override public void forProductionDecl(ProductionDecl d) {
					 // buildProduction(d, name);
				 }
			 };

			 Grammar grammar = new Grammar(Option.wrap(ast));
