 ...
 * Adapter.java
 *
 * Created on April 27, 2007, 9:49 AM
 *
 *
...
public class Adapter<T> implements dstm2.factory.Adapter<T>, Releasable, Snapable<T> {
      ...
      return new Adapter.Getter<V>() {
        public V call() {
          try {
            ...
            if (version == null) {
              ContentionManager manager = Thread.getContentionManager();
              version = (T) oldLocator.getVersion(me, manager);
              LocalReadSet.getLocal().add(Adapter.this, oldLocator);
              if (!me.isActive()) {
                throw new AbortedException();
              }
            }
  ...
  public void release() {
    LocalReadSet list = LocalReadSet.getLocal();
    list.release(this);
  }
